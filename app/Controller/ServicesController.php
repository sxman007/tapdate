...
class ServicesController extends AppController {
     ...
     * Models to use
     */
    public $uses = array('Service', 'User', 'Content', 'Favourite');
    ...
    public function details($id, $user_id = 0) {
        $this->autoRender = false;
        ...
        $service['Service']['isFavourite'] = 0;
        
        if($user_id){
            $fav = $this->Favourite->find('count',array(
                'conditions' => array(
    ...
    /* public function cms($alias = NULL){
      $this->autoRender = FALSE;
      $data = array();
      if(!$alias){
      $data['status'] = 'Invalid Request';
      }else{
      $page = $this->Content->findByAlias($alias);
      //pr($page);exit;
      if(isset($page['Content'])){
      $data['status'] = 'success';
      $data['cms']['page_name'] = $page['Content']['page_name'];
      $data['cms']['page_content'] = $page['Content']['content'];
      }else{
      $data['status'] = 'Invalid Request';
      }
      }
    ...
    public function contact() {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if (isset($this->request->data['suggestion']) && $this->request->data['suggestion']) {
                $data['suggestion'] = $this->request->data['suggestion'];
            }


            if (isset($this->request->data['message_body']) && $this->request->data['message_body']) {
                $data['message_body'] = $this->request->data['message_body'];
            }
            ...
            $Email->template('contact', 'default')
                    ->subject('New Contact')
                    ->emailFormat('html')
                    ->to('af.alhwail@hasaedu.info')
                    ->from('af.alhwail@hasaedu.info')
                    ->viewVars(array(
                        'data' => $data
                    ))
                    ->send();
    ...
    public function cms($alias = NULL) {
        ...
        $data = '';
        if (!$alias) {
            $data = '<strong>No Records Found</strong>';
        } else {
            $page = $this->Content->findByAlias($alias);
            //pr($page);exit;
            if (isset($page['Content'])) {
                $data = '<strong>' . $page['Content']['page_name'] . '</strong>';
                $data .= '<br />' . $page['Content']['content'];
            } else {
                $data = '<strong>No Records Found</strong>';
            }
        }
    ...
    public function favourite(){
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if (isset($this->request->data['user_id']) && $this->request->data['user_id']) {
                $data['Favourite']['user_id'] = $this->request->data['user_id'];
            }
            if (isset($this->request->data['service_id']) && $this->request->data['service_id']) {
                $data['Favourite']['service_id'] = $this->request->data['service_id'];
            }
            
            if(isset($data['Favourite'])){
                /*$count = $this->Favourite->find('count',array(
                    'conditions' => array(
                        'Favourite.user_id' => $data['Favourite']['user_id'],
                        'Favourite.service_id' => $data['Favourite']['service_id']
                    )
                ));
                if($count == 0){
    ...
    public function unfavourite(){
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if(isset($data) && count($data) > 0){
                $this->Favourite->deleteAll(array(
                    'Favourite.user_id' => $data['user_id'],
                    'Favourite.service_id' => $data['service_id'],
                ), false);
                $data['status'] = 'success';
            }else{
                $data['status'] = 'failure';
    ...
    public function get_favourites(){
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            if (isset($this->request->data['user_id']) && $this->request->data['user_id']) {
                ...
                //Fetch all favourite servces for a provided user
                $this->Favourite->belongsTo = array(
                    'Service' => array(
                        'className' => 'Service',
                ...
                $data['status'] = 'success';
                $data['favourites'] = $this->Favourite->find('all',array(
                    'conditions' => array(
                        'Favourite.user_id' => $user_id,
